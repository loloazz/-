...
def get_title(res):
    ...
    url = parse.xpath('/html/body/div[2]//@href')
...
def proms(urls,heards):
    fp = open("古诗/诗句初选/文言文.txt", 'w', encoding='utf-8')
    ...
    for i in urls:
            ...
            # 使用xpath匹配文章标签xpath()返回为一列表  @修饰属性  /text() 获取当前路径下的文本内容
            title = parse.xpath("/html/body/div[2]/div[1]/div[2]/div[1]/h1/text()")
            for i in title:
                fp.write(i+" ")

        #    print(title)
            # enumerate() 函数用于将一个可遍历的数据对象(如列表、元组或字符串)组合为一个索引序列，同时列出数据和数据下标
            auther = parse.xpath("ml/body/div[2]/div[1]/div[2]/div[1]/p/a/text()")
            for i in auther:
                fp.write(i)
...
if __name__ == '__main__':
    url = 'https://so.gushiwen.cn/wenyan/guanzhi.aspx'
    headers = {
